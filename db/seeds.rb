...
Parallel.each(cleaned_movie_now_playing_data, in_processes: threads, progress: "Creating movies") do |movie_data|
    begin
        ...
        in_depth_movie_data = JSON.parse(RestClient.get("#{base_api}#{movie_end_point}/#{movie_id}#{base_api_query}&append_to_response=credits"))
        movie_credits = in_depth_movie_data["credits"]
        
        movie = Movie.create(movie_db_id: movie_id, title: in_depth_movie_data["title"], description: in_depth_movie_data["overview"], original_title: in_depth_movie_data["original_title"], in_greek_theatres: true)
